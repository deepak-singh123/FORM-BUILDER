...
const Cloze = ({ Qno, question }) => {
  ...
  useEffect(() => {
    if (question) {
      ...
      setUnderlinedWords(question.underlinedWords || []);
      setPreviewText(question.preview || question.sentence || "");
  ...
  const handleCheckboxChange = (index) => {
    const updatedWords = [...underlinedWords];
    updatedWords[index].isselected = !updatedWords[index].isselected;
    setUnderlinedWords(updatedWords);
  };
...
const handleAddOption = () => {
  ...
  useEffect(() => {
    saveDataToRedux();
  }, [points, previewText, sentence, underlinedWords]);
  return (
      ...
      <div className="underline-btn-container">
        ...
        {sentence && (
          <button
            ...
            onClick={() => handleUnderlineWord(selectedWords)}
          >
            <PiTextUnderlineFill size={40} />
          </button>
        )}
